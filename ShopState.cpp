...
void ShopState::ComputeHudInformation()
{
	if(playerShip)
	{
		...
		hud->SetBarPercent(HUD::SHIELD, playerShip->GetShield() / SaiphApp::GetShieldFlyweight(SaiphApp::GetShieldID()).strength); //// Fixed
...
void ShopState::ResetPlayer()
{
	...
	playerShip->SetYPosition( WINDOW_HEIGHT / 2.0f); //// fixed
...
void ShopState::FireWeapon()
{
	...
	if((playerShip->GetBattery() > wlf.energy) && (playerShip->GetLastFired() >= wlf.reloadTime)) ///// Fixed
	{
	...
	if(_menu)
	{
		...
		if(selected)
		{
			...
			else if(submenu == "Weapon")
			{
				...
				{
					temp = atoi(selected->GetActionValue().c_str());
					SaiphApp::SetWeaponID(temp / 100); ///// fixed
...
void ShopState::Init()
{
	...
	// mouse
	cursor = CreateMouseCursor(); /// Fixed
...
void ShopState::RenderFrame() const
{
	...
	if(alteredScore == (int)SaiphApp::GetScore())  //// Fixed
...
void ShopState::Shutdown()
{
	...
	// shiptype data
	for (std::map<int, Ship*>::iterator iter = shiptypes.begin(); iter != shiptypes.end();) //// Fixed
	{
