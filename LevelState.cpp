...
#include "DeflectionLab/DeflectionLab.h"
...
//////////// Added code for the lab/////////////////////

/////// Create Mouse Cursor ///////////////
BaseObject* LevelState::CreateMouseCursor()
{

	BaseObject* mouseCursor = new BaseObject();
	ViewManager::GetInstance().AddObject(mouseCursor, 5);
	mouseCursor->SetTexture("Resources/images/reticle.png");
	mouseCursor->SetDimensions(32, 32);
	mouseCursor->SetColor(0.74f, 1, 1, 1);
	return mouseCursor;
}

/////// Create HUD ///////////////
HUD* LevelState::CreateHUD()
{
	HUD* Hud = new HUD();
	return Hud;
}
...
/////// Create Progress Bar ///////////////
ProgressBar* LevelState::CreateProgressBar()
{
	ProgressBar* progress = new ProgressBar();
	progress->SetTexture("Resources/images/progressBar2.png");
	progress->SetDimensions(32.0f, 8.0f);

	return progress;
}

/////// Create Player Ship ///////////////
Ship* LevelState::CreatePlayerShip()
{

	
	// ship creation
	Ship* ship;
	ship = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID());
...
/////// Create BackGround ///////////////
BaseObject* LevelState::CreateBackground()
{
...
/////// Create BackGround ///////////////
PowerUp* LevelState::CreatePowerUp()
{
	PowerUp* power = new PowerUp();
	ViewManager::GetInstance().AddObject(power, 2);
	return power;
}
...
void LevelState::GeneratePowerUps(Enemy* _enemy)
{
	...
	{
		PowerUp* power = CreatePowerUp();
		power->SetDimensions(32, 32);
		int poweruptype = _enemy->GetPowerUpID();
		if (poweruptype == 0)
		{
		
			power->SetTexture("Resources/images/pupscore.png");
			power->SetType(0);
		}
		else if (poweruptype == 1)
		{
		
			power->SetTexture("Resources/images/pupenergy.png");
			power->SetType(1);
		}
		else if (poweruptype == 2)
		{
			
			power->SetTexture("Resources/images/puparmor.png");
			power->SetType(2);
		}
		else if (poweruptype == 3)
		{
			
			power->SetTexture("Resources/images/pupwaepon.png");
			power->SetType(3);
		}
		...
		power->SetVelocity(randomVelocity);
		power->SetPosition(_enemy->GetShip()->GetXPosition(), _enemy->GetShip()->GetYPosition());
...
void LevelState::ResetPlayer()
{
	...
	/// LAB 5 Create Player Ship /////
	
	Ship* tempship = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID());
	MotherShip* player = new MotherShip(*tempship);
	ViewManager::GetInstance().RemoveObject(tempship);
	delete tempship;
	ViewManager::GetInstance().AddObject(player, 2);
	playerShip = dynamic_cast<MotherShip*> (player);
	...
	for (int i = 0; i < 5; i++)
	{
		Ship* newShadow = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID(), 1);
		newShadow->SetColor(0.5f, 0.50f, 0.50f, 0.50f);
		player->AddShadow(newShadow);
	}
	...
	if (!gamePaused)
	{
		...
		if (playerShip)
		{
			...
			if (GetAsyncKeyState('C') & 0x1)
			{
				...
				for (cur = 1; cur < enemies.size(); ++cur)
				{
					if (playerShip->GetBattery() >= 10 )
					{
						if (enemies[cur]->GetShip()->Collide(*cursor))
						{
							BoomBoom(enemies[cur]);
			...
			if (GetAsyncKeyState('X') & 0x1)
			{
				...
				for (cur = 0; cur < enemies.size(); cur++)
				{
					enemies[cur]->NotifyObservers(Observer::ObserverEvent::ACTIVATE);
				}
		...
		{
			if (!playerShip->GetDeflector())
			{
				Deflector* shield = new Deflector(playerShip, this);
				playerShip->SetDeflector(shield);
			}
			else
		...
		{
			playerShip->GetDeflector()->ActivateStrategy("Debug/DEFLECTIONLAB.dll");
		}

		if (GetAsyncKeyState('G') & 0x1)
		{
			playerShip->GetDeflector()->ActivateStrategy("Debug/DLLSTRAT.dll");
		}
	}
...
void LevelState::Shutdown()
{
	...
	delete playerShip->GetDeflector();
...
void LevelState::BoomBoom(Enemy* _enm)
{
	StickyBomb* bomb = new StickyBomb();
	bomb->SetPosition(playerShip->GetPosition());
	bomb->SetHeading(_enm->GetShip()->GetHeading());
	bomb->SetTarget(_enm);
