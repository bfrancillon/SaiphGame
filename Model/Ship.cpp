...
void Ship::SetDeflector(Deflector* def) { deflector = def; }
...
Deflector* Ship::GetDeflector() { return deflector; }
...
bool Ship::Collide(Bullet& _in)
{
	bool collide = false;

	if (!deflector)
		collide = BaseObject::Collide(_in);
	else if (deflector->Collide(_in))
	{
